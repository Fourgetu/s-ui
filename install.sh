...
#   bash <(curl -Ls https://raw.githubusercontent.com/Fourgetu/s-ui/main/install.sh)            # auto (default)
#   SUI_AUTO=0 bash <(curl -Ls https://raw.githubusercontent.com/Fourgetu/s-ui/main/install.sh) # interactive
SUI_AUTO="${SUI_AUTO:-1}"
...
install_s-ui() {
    ...
    if [ $# == 0 ]; then
        last_version=$(curl -Ls "https://api.github.com/repos/Fourgetu/s-ui/releases/latest" | grep '"tag_name":' | head -1 | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        if [[ -z "$last_version" ]]; then
            last_version=$(curl -Ls "https://api.github.com/repos/Fourgetu/s-ui/releases" | grep '"tag_name":' | head -1 | sed -E 's/.*"([^"]+)".*/\1/')
        fi
        ...
        if ! download_release /tmp/s-ui-linux-$(arch).tar.gz "https://github.com/Fourgetu/s-ui/releases/download/${last_version}/s-ui-linux-$(arch).tar.gz"; then
            echo -e "${red}$(t download_fail)${plain} (direct download and mirrors were tried; also check free space in /tmp)"
            exit 1
        fi
    else
        last_version=$1
        url="https://github.com/Fourgetu/s-ui/releases/download/${last_version}/s-ui-linux-$(arch).tar.gz"
